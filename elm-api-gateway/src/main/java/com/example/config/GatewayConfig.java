...
public class GatewayConfig implements GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
     ...
     // Log all incoming request headers
        ...
        // ✅ Apply Basic Authentication ONLY to employee-service requests
        if (request.getURI().getPath().startsWith("/employees/")) {
     ...
     // ✅ If JWT is present, forward it to backend services
        ...
        return chain.filter(exchange.mutate().request(exchange.getRequest()).build())
                .doOnError(error -> {
                    System.err.println("❌ Gateway Filter Error: " + error.getMessage());
                    error.printStackTrace(); // Log full error stack trace
                });
    }
}
