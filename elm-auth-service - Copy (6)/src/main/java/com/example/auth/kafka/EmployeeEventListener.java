...
public class EmployeeEventListener {
    ...
    public void handleEmployeeEvents(EmployeeEvent event) {
        ...
        switch (event.getEventType()) {
            case EMPLOYEE_CREATED -> createAuthUserFromEmployee(event);
    ...
    private void createAuthUserFromEmployee(EmployeeEvent event) {
        // Your existing creation logic
        // Example:
        AuthUserDTO dto = new AuthUserDTO(
            null,
            (event.getEmployee().getFirstName() + "." + event.getEmployee().getLastName()).toLowerCase(),
            event.getEmployee().getEmail(),
            "defaultPassword123",
            mapDepartmentToRole(event.getEmployee().getDepartment())
        );
